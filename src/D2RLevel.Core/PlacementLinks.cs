...
public sealed class PlacementLinks
{
    ...
    public PlacementLinks(PresetDocument json, Ds1CollisionDocument ds1, string? sidecarPath = null)
    {
        ...
        file = new(1, Path.GetRelativePath(Path.GetDirectoryName(SidecarPath)!, ds1.SourcePath), ds1.LinkFingerprint(), [], []);
        try
        {
    ...
    private PresetEntity Entity(PlacementLink link)
    {
        var matches = json.Entities.Where(e => e.Id == link.EntityId).ToArray();
        if (matches.Length != 1 || matches[0].PreviewModel != link.Model || matches[0].HasParent || !matches[0].CanTransform)
            throw new InvalidDataException($"HD identity changed for {link.Name}.");
        return matches[0];
    }
    private void ValidateFile()
    {
        if (file.Version != 1 || file.Links is null || file.Baselines is null || file.Fingerprint != ds1.LinkFingerprint())
        ...
        {
            link.Anchor.Validate(); ValidateScale(link.UnitsPerTile);
        ...
        foreach (var tile in Owned())
            if (!file.Baselines.Any(b => b.X == tile.X && b.Y == tile.Y)) throw new InvalidDataException("Missing footprint baseline; open the sidecar exported with this pair.");
    }
    private static void ValidateScale(double scale)
    { if (!double.IsFinite(scale) || scale < .01 || scale > 10000) throw new InvalidDataException("Invalid HD units/tile scale."); }
    ...
    private LinkedTile[] Tiles(PlacementLink link, EntityTransform? value = null)
    {
        var t = value ?? Entity(link).Transform;
    ...
    private HashSet<LinkedTile> Owned(PresetEntity? changing = null, EntityTransform? value = null) =>
        file.Links.SelectMany(l => Tiles(l, l.EntityId == changing?.Id ? value : null)).ToHashSet();
    private void ValidateCurrent(PlacementLink link)
    {
        var entity = Entity(link);
        ...
        foreach (var tile in Tiles(link)) if (!ds1.FloorOverride(tile)) throw new InvalidDataException($"{link.Name}: owned collision was changed outside the linked workspace.");
    }
    private void Activate()
    {
        ...
        ds1.ProtectedUnit = index => file.Links.Any(l => l.Unit?.Index == index);
    }
    public void ConnectWorkspace() { EnsureReady(); Activate(); }
    public byte[] MetadataFor(string sidecarPath, string ds1Path)
    {
        EnsureReady(); ValidateFile();
        return System.Text.Encoding.UTF8.GetBytes(Serialize(file with
    ...
    { EnsureReady(); return Find(entity) is { } link ? Tiles(link) : []; }
    ...
    public IReadOnlyDictionary<LinkedTile, CollisionOwnership> InspectOwnershipForTiles(IEnumerable<LinkedTile> cells)
    {
        ...
        var owners = file.Links.SelectMany(l => Tiles(l).Where(requested.Contains).Select(t => (Tile: t, l.Name)))
    ...
    {
        EnsureReady(); ValidateScale(unitsPerTile); entity.Transform.Validate();
        ...
        if (file.Links.Any(l => l.Unit?.Index == index)) throw new InvalidOperationException("This DS1 placement already belongs to another HD object.");
    ...
    public void LinkFootprint(PresetEntity entity, IEnumerable<LinkedTile> cells, double unitsPerTile, bool claimExisting)
    {
        EnsureReady(); ValidateScale(unitsPerTile); entity.Transform.Validate();
        var existing = Find(entity);
        foreach (var current in file.Links) ValidateCurrent(current);
        ...
        var nextOwned = nextLinks.SelectMany(l => Tiles(l)).ToHashSet();
    ...
    {
        EnsureReady();
        var link = Find(entity) ?? throw new InvalidOperationException("Link a DS1 unit first.");
        ValidateCurrent(link);
    ...
    {
        EnsureReady(); ValidateFile(); Activate();
        var link = Find(entity);
        var remaining = file.Links.Where(l => l.EntityId != entity.Id).ToArray();
        var owned = remaining.SelectMany(l => Tiles(l)).ToHashSet();
        var baseline = file.Baselines.ToDictionary(b => new LinkedTile(b.X, b.Y));
        var states = (link is null ? [] : Tiles(link)).Select(t => new TileBaseline(t.X, t.Y, owned.Contains(t) || baseline[t].Blocked)).ToArray();
        json.History.Transaction(() =>
        {
            ds1.SetFloorOverrides(states);
            if (link?.Unit is { } unit)
    ...
    public void Unlink(PresetEntity entity)
    {
        ...
        var stationary = Tiles(link).ToHashSet();
        ChangeMetadata(file with { Links = file.Links.Where(l => l.EntityId != entity.Id).ToArray(),
    ...
    {
        var previous = file;
        void Set(PlacementLinkFile value) { file = value; HasMetadataChanges = Serialize(file) != savedMetadata; }
    ...
    public void Move(PresetEntity entity, EntityTransform target)
    {
        ...
        foreach (var existing in file.Links) ValidateCurrent(existing);
        ...
        json.History.Transaction(() =>
        {
            ...
            file = after;
            json.History.Record(() => file = before, () => file = after);
    ...
    public void SaveMetadata(string? path = null, string? ds1Path = null)
    {
        ...
        var output = file with { Ds1Path = Path.GetRelativePath(Path.GetDirectoryName(Path.GetFullPath(path))!, ds1Path ?? ds1.SourcePath) };
        ...
        try
        {
            ...
            if (path == SidecarPath) { savedMetadata = Serialize(file); HasMetadataChanges = false; }
        }
    ...
    public static string? LinkedDs1Path(string jsonPath)
    {
        ...
        if (data.Version != 1 || string.IsNullOrWhiteSpace(data.Ds1Path) || !data.Ds1Path.EndsWith(".ds1", StringComparison.OrdinalIgnoreCase)) throw new InvalidDataException("Invalid linked DS1 path.");
