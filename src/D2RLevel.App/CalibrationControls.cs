...
public partial class MainWindow
{
    ...
    private void InitializeCalibration()
    {
        ...
        Status.Text = L.T("Grid calibration is still the unverified default. Terrain in this scene measures {0} Open Calibrate… to review and apply it.", measured.Describe());
    }
    ...
    private void ApplyCalibration(GridCalibration value, bool announce = true)
    {
        ...
        if (placementLinks is { Warning: null } links)
        {
            ...
            { Status.Text = L.T("Calibration applied to this session only: {0}", ex.Message); RefreshState(); return; }
        }
        RefreshLinkStatus(); RefreshState();
        if (announce)
            Status.Text = L.T("Grid calibration: {0}", value.Describe()) +
                (placementLinks is { Warning: null } ? " " + L.T("Save Scene or Save linked pair to keep it with this pair.") : "");
    }

    private void Calibrate_Click()
    {
        if (pairedScene is null) { Status.Text = L.T("Open the paired DS1 before calibrating."); return; }
        var dialog = new Window
        {
            Title = L.T("Grid calibration"), Owner = this, Width = 560, SizeToContent = SizeToContent.Height,
        ...
        Line(L.T("How many HD units span one DS1 tile in this scene?"), bold: true);
        Line(L.T("Unit links, footprint suggestions and NPC placement all divide by this number. Objects already linked keep the scale they were created with, so changing it here cannot move them."),
             new SolidColorBrush(Color.FromRgb(154, 175, 196)));
        Line(L.T("Current: {0}", current.Describe()));
        ...
        void Offer(GridCalibration value)
        {
            ...
            result.Text = L.T("Proposed: {0}", value.Describe()) + (value.Warning is null ? "" : "\n" + value.Warning);
        }
        ...
        Action(L.T("Measure from terrain"), L.T("Compare the terrain mesh extent with the DS1 tile grid."),
            () => MeasureTerrainCalibration() ?? throw new InvalidOperationException(
                L.T("This scene has no decoded terrain to measure. Load the asset folder and terrain, or use another method.")));
        Action(L.T("Solve from existing links"), L.T("Fit the scale to every object already linked to a DS1 placement."),
            () => (placementLinks ?? throw new InvalidOperationException(L.T("Load the JSON and matching DS1 first."))).SolveCalibrationFromLinks());

        var manual = new WrapPanel { Margin = new Thickness(0, 0, 0, 8) }; panel.Children.Add(manual);
        manual.Children.Add(new TextBlock { Text = L.T("Or enter it directly:"), VerticalAlignment = VerticalAlignment.Center, Margin = new Thickness(0, 0, 8, 0) });
        ...
        var useTyped = new Button { Content = L.T("Use this value"), Padding = new Thickness(9, 5, 9, 5), Margin = new Thickness(8, 0, 0, 0) };
        useTyped.Click += (_, _) =>
        {
            try
            {
                if (!double.TryParse(typed.Text, NumberStyles.Float, CultureInfo.InvariantCulture, out double value))
                    throw new InvalidDataException(L.T("Enter a number, for example 10."));
        ...
        var apply = new Button { Content = L.T("Apply"), Padding = new Thickness(14, 5, 14, 5), Margin = new Thickness(0, 0, 8, 0), Background = new SolidColorBrush(Color.FromRgb(40, 104, 108)) };
        apply.Click += (_, _) =>
        {
            if (chosen is null) { Fail(L.T("Choose or enter a value first.")); return; }
            dialog.DialogResult = true;
        };
        var cancel = new Button { Content = L.T("Cancel"), Padding = new Thickness(14, 5, 14, 5), IsCancel = true };
