...
3. Entrance/exit inspection and supported authoring are now implemented. Ordinary exits use orientations 10/11 and main indices 0–7; the town's main 30–33 markers have special placement roles and are not exit slots. The editor inspects outgoing/incoming routes and Warp definitions, moves supported hidden groups, and stages reciprocal connections between validated fixed presets. See [mapping evidence, guards and workflow](entrances-and-exits.md). Arbitrary portals and generated-area authoring remain open.
